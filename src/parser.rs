...
/// Token lexer
fn lexer<'src>() -> impl Parser<'src, &'src str, Vec<Spanned<Token<'src>>>, extra::Err<Rich<'src, char, CSpan>>> {
    // integers and floats
    ...
    let boolean = just("#t").to(Token::Bool(true))
    ...
    let token = float.or(int).or(boolean).or(string).or(ident).or(lparen).or(rparen);
...
fn expr_parser<'tokens, 'src: 'tokens, I>() -> impl Parser<'tokens, I, Expression, extra::Err<Rich<'tokens, Token<'src>, CSpan>>> + Clone
where
    I: ValueInput<'tokens, Token = Token<'src>, Span = CSpan>,
{
    recursive(|expr| {
        let atom = select! {
            ...
            .map_with(|b, e| { let s: CSpan = e.span(); Expression::boolean(b, Span::new(s.start, s.end)) })
            .or(select! { Token::Int(i) => i }
                .map_with(|i, e| { let s: CSpan = e.span(); Expression::integer(i, Span::new(s.start, s.end)) }))
            .or(select! { Token::Float(f) => f }
                .map_with(|f, e| { let s: CSpan = e.span(); Expression::float(f, Span::new(s.start, s.end)) }))
            .or(select! { Token::Str(s) => s }
                .map_with(|s, e| { let sp: CSpan = e.span(); Expression::string(s.to_string(), Span::new(sp.start, sp.end)) }))
            .or(select! { Token::Ident(id) => id }
                .map_with(|id, e| { let sp: CSpan = e.span(); Expression::variable(id.to_string(), Span::new(sp.start, sp.end)) }));
        ...
        let list = expr
            ...
            .map_with(|elements, e| {
                ...
                let keyword = match &elements[0] {
                    Expression::Literal { value: Value::Symbol(s), .. } => s.clone(),
                    Expression::Variable { name, .. } => name.clone(),
                    _ => String::new(),
                };
                match keyword.as_str() {
                    "define" => {
                        if elements.len() < 3 { return Expression::nil(span); }
                        if let Expression::Call { .. } = &elements[1] {
                            ...
                            if let Expression::Call { function, arguments, .. } = &elements[1] {
                                if let Expression::Variable { name, .. } = &**function {
                                    let params = arguments.iter().map(|arg| {
                                        if let Expression::Variable { name, span } = arg {
                                            Parameter { name: name.clone(), type_annotation: None, span: *span }
                                        } else {
                                            Parameter { name: String::new(), type_annotation: None, span: Span::default() }
                                        }
                                    }).collect();
                                    return Expression::DefineFunction { name: name.clone(), parameters: params, body: elements[2..].to_vec(), span };
                                }
                            }
                        }
                        if let Expression::Variable { name, .. } = &elements[1] {
                            Expression::Define { name: name.clone(), value: Box::new(elements[2].clone()), span }
                        } else if let Expression::Literal { value: Value::Symbol(name), .. } = &elements[1] {
                            Expression::Define { name: name.clone(), value: Box::new(elements[2].clone()), span }
                        } else {
                            Expression::Call { function: Box::new(elements[0].clone()), arguments: elements[1..].to_vec(), span }
                        }
                    }
                    "lambda" => {
                        if elements.len() < 3 { return Expression::nil(span); }
                        ...
                        if let Expression::Literal { value: Value::List(param_values), .. } = &elements[1] {
                            for p in param_values {
                                if let Value::Symbol(n) = p { params.push(Parameter { name: n.clone(), type_annotation: None, span: Span::default() }); }
                            }
                        }
                        Expression::Lambda { parameters: params, body: elements[2..].to_vec(), span }
                    }
                    "if" => {
                        let condition = elements.get(1).cloned().unwrap_or_else(|| Expression::boolean(false, Span::default()));
                        let then_expr = elements.get(2).cloned().unwrap_or_else(|| Expression::nil(Span::default()));
                        let else_expr = elements.get(3).cloned();
                        Expression::If { condition: Box::new(condition), then_expr: Box::new(then_expr), else_expr: else_expr.map(Box::new), span }
                    }
                    "let" => {
                        if elements.len() < 3 { return Expression::nil(span); }
                        ...
                        for pair_expr in pairs {
                            if let Expression::Call { function, arguments, span: pair_span } = pair_expr {
                                if let Expression::Variable { name, .. } = *function {
                                    if arguments.len() == 1 {
                                        bindings.push(Binding { name, value: arguments[0].clone(), span: pair_span });
                                    }
                                }
                            }
                        }
                        Expression::Let { bindings, body: elements[2..].to_vec(), span }
                    }
                    "begin" => Expression::Begin { expressions: elements[1..].to_vec(), span },
                    "quote" => {
                        if elements.len() < 2 { return Expression::nil(span); }
                        Expression::Quote { expression: Box::new(elements[1].clone()), span }
                    }
                    _ => Expression::Call { function: Box::new(elements[0].clone()), arguments: elements[1..].to_vec(), span },
                }
            })
...
pub fn parse(source: &str) -> Result<Program> {
    ...
    if let Some(tokens) = &tokens {
        let (exprs, parse_errs) = expr_parser()
            .repeated()
            .collect::<Vec<_>>()
            .parse(tokens.as_slice().map((preprocessed.len()..preprocessed.len()).into(), |(t, s)| (t, s)))
            .into_output_errors();
        if !lex_errs.is_empty() || !parse_errs.is_empty() {
            if let Some(e) = parse_errs.first() {
                let sp = e.span();
                return Err(MoriaError::Parse { message: format!("{:?}", e), span: Span::new(sp.start, sp.end), context: None });
        ...
        if let Some(expressions) = exprs {
            let span = if expressions.is_empty() { Span::default() } else { let s = expressions.first().unwrap().span().start; let e = expressions.last().unwrap().span().end; Span::new(s, e) };
            return Ok(Program { expressions, span });
        }
    }
    Err(MoriaError::Parse { message: "Failed to parse program".to_string(), span: Span::new(0, len), context: None })
}
...
pub fn parse_expression(source: &str) -> Result<Expression> {
    ...
    if let Some(tokens) = &tokens {
        ...
        if let Some((expr, _span)) = expr { return Ok(expr); }
    }
    Err(MoriaError::Parse { message: "Failed to parse expression".to_string(), span: Span::new(0, len), context: None })
}
